...
parser = argparse.ArgumentParser(description='Run live demo of depth completion on RGB camera')
...
while True:
    ...
    grid_image1 = np.concatenate((color_img, outlines_rgb, occlusion_weight_rgb), 1)
    grid_image2 = np.concatenate((np.zeros(color_img.shape, dtype=color_img.dtype)), 1)
...
if __name__ == '__main__':
   ...
   # Save a copy of config file in the logs
    ...
    outputImgHeight = int(config.depth2depth.yres)
    outputImgWidth = int(config.depth2depth.xres)
    depthcomplete = depth_completion_api.DepthToDepthCompletion(masksWeightsFile=config.masks.pathWeightsFile,
                                                                masksModel=config.masks.model,
                                                                outputImgHeight=outputImgHeight,
                                                                outputImgWidth=outputImgWidth,
                                                                fx=int(config.depth2depth.fx),
