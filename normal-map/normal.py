...
parser.add_argument('normalmaps_output_dir', nargs='?', default="/../../media/kattun/HD-PGF-A/bag-img-data/spacer-bag/normal-maps",  help="Path to where the final files, will be saved")
...
for i in range(rq):
    ...
    bproc.renderer.enable_normals_output(output_dir = args.normalmaps_output_dir +'/')
    ...
    #ファイル名セッティング
    file_name = filename_setting(i-5+num)
    
    #output normal image
    for index, image in enumerate(data["normals"]):   
        #save_array_as_image(image, "normals", os.path.join(args.normalmaps_output_dir,str(file_name) +"-normalmap"  + ".png"))
        imageio.imwrite(os.path.join(args.normalmaps_output_dir) +'/' + str(file_name) +'normals.exr', image[1])
